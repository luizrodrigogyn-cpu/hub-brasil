...
type Installer = { id: number; name: string; city: string; state: string; specialties: string[]; serviceStates: string[]; description?: string | null; phonePreview: string };
...
export default function InstallersPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch("/api/installers").then(readJson),
      fetch("/api/me", { cache: "no-store" }).then(readJson),
    ]).then(([directory, me]) => { setRows(directory.installers || []); setAuthenticated(Boolean(me.authenticated)); }).catch(() => setMessage("Não foi possível carregar o diretório agora."));
  }, []);
  ...
  async function register(event: FormEvent<HTMLFormElement>) {
    ...
    if (!response.ok) setMessage(result.error || "Não foi possível enviar o cadastro.");
    else { setMessage("Cadastro enviado. O perfil aparecerá após validação do WhatsApp e aprovação da gestão."); setFormOpen(false); event.currentTarget.reset(); }
    setBusy(false);
  }
  ...
  return <main className="installers-page">
    ...
    {formOpen && <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setFormOpen(false); }}><section className="access-modal installer-form" role="dialog" aria-modal="true" aria-labelledby="installer-form-title"><button className="modal-close" onClick={() => setFormOpen(false)} aria-label="Fechar">×</button><span className="eyebrow">CADASTRO PROFISSIONAL</span><h2 id="installer-form-title">Quero aparecer no diretório</h2><p>Não solicitamos CPF, endereço residencial ou valores de serviço.</p>{!authenticated ? <div className="installer-login"><p>Entre por código de e-mail antes de preencher o cadastro.</p><a className="primary full" href="/sign-in?return_to=/instaladores">Entrar com segurança →</a></div> : <form onSubmit={register}><label>Nome profissional ou empresa<input name="name" required maxLength={120} /></label><label>WhatsApp com DDD<input name="phone" required inputMode="tel" placeholder="(00) 00000-0000" /></label><div className="field-row"><label>Cidade<input name="city" required maxLength={100} /></label><label>UF<select name="state" required><option value="">Selecione</option>{BRAZIL_STATES.map((uf) => <option key={uf}>{uf}</option>)}</select></label></div><fieldset className="solution-selector"><legend>Especialidades <small>selecione uma ou mais</small></legend>{SPECIALTIES.map((item) => <label className="check" key={item}><input type="checkbox" name="specialties" value={item} /> {item}</label>)}</fieldset><fieldset className="solution-selector"><legend>Outras UFs atendidas <small>opcional</small></legend>{BRAZIL_STATES.map((uf) => <label className="check" key={uf}><input type="checkbox" name="serviceStates" value={uf} /> {uf}</label>)}</fieldset><label>Apresentação <small>opcional</small><textarea name="description" rows={4} maxLength={800} placeholder="Experiência, serviços e região atendida" /></label><label className="consent"><input type="checkbox" name="contactConsent" required /><span>Autorizo a publicação do meu nome profissional, cidade, UF, especialidades e a liberação do WhatsApp para usuários autenticados que solicitarem contato.</span></label><label className="consent"><input type="checkbox" required /><span>Concordo com os <a href="/termos" target="_blank">Termos</a> e a <a href="/privacidade" target="_blank">Política de Privacidade</a>.</span></label><button className="primary full" disabled={busy}>{busy ? "Enviando…" : "Enviar para aprovação →"}</button></form>}</section></div>}
  </main>;
}
