...
export async function POST(request: Request) {
  ...
  if (body.entity === "supplier") {
    ...
    if (body.action === "approve") { const [event] = await db.select().from(supplierEvents).where(eq(supplierEvents.id, body.id)); const [supplier] = event?.ownerUserId ? await db.select().from(leads).where(eq(leads.authUserId, event.ownerUserId)) : []; if (!supplier || supplier.status !== "approved" || !supplier.phoneVerifiedAt) return Response.json({ error: "O fornecedor ainda não está aprovado." }, { status: 400 }); await db.update(supplierEvents).set({ status: "approved" }).where(eq(supplierEvents.id, body.id)); }
