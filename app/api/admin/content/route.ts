...
export async function POST(request: Request) {
  ...
  if (body.entity === "installer") {
    ...
    const approveProduct = async (productId: number) => { const [product] = await db.select().from(products).where(eq(products.id, productId)); const [supplier] = product?.ownerUserId ? await db.select().from(leads).where(eq(leads.authUserId, product.ownerUserId)) : []; if (!product) throw new Error("Produto não encontrado."); if (!supplier || supplier.status !== "approved" || !supplier.phoneVerifiedAt) throw new Error(`O fornecedor de ${product.name} ainda não está aprovado.`); await db.update(products).set({ status: "approved" }).where(eq(products.id, productId)); await awardCredit(db,{supplierId:supplier.id,ruleKey:"product_approved",sourceType:"product",sourceId:productId,idempotencyKey:`product-approved:${productId}`}); await recomputeHubScore(db,supplier.id,"produto_aprovado"); await qualifyReferralIfReady(db,supplier.id); };
