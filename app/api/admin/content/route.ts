...
export async function GET() {
  ...
  const [suppliers, registrationRows, installerRows, productRows, events, needs, updates, articles, news, reports, deletions, audit] = await Promise.all([
    ...
    db.select({ id: leads.id, authUserId: leads.authUserId, name: leads.name, company: leads.company, phone: leads.phone, phoneEncrypted: leads.phoneEncrypted, role: leads.role, status: leads.status, createdAt: leads.createdAt }).from(leads).orderBy(desc(leads.createdAt)),
...
export async function POST(request: Request) {
  ...
  const body = await request.json() as { entity?: string; id?: number; ids?: number[]; action?: string; value?: string; title?:string; summary?:string; content?:string; category?:string; author?:string; sourceName?:string; sourceUrl?:string; imageUrl?:string; publishedAt?:string };
  if (!body.entity || !body.action || (!['create_article', 'create_news', 'approve_many'].includes(body.action) && !body.id)) return Response.json({ error: "Ação inválida." }, { status: 400 });
  ...
  if (body.entity === "installer") {
    ...
    const approveProduct = async (productId: number) => { const [product] = await db.select().from(products).where(eq(products.id, productId)); if (!product) throw new Error("Produto não encontrado."); const [supplier] = product.supplierId ? await db.select().from(leads).where(eq(leads.id, product.supplierId)) : product.ownerUserId ? await db.select().from(leads).where(eq(leads.authUserId, product.ownerUserId)) : []; if (!supplier || supplier.status !== "approved") throw new Error(`O fornecedor de ${product.name} ainda não está aprovado.`); await db.update(products).set({ status: "approved", supplierId: supplier.id }).where(eq(products.id, productId)); await awardCredit(db,{supplierId:supplier.id,ruleKey:"product_approved",sourceType:"product",sourceId:productId,idempotencyKey:`product-approved:${productId}`}); await recomputeHubScore(db,supplier.id,"produto_aprovado"); await qualifyReferralIfReady(db,supplier.id); };
    if (body.action === "approve_many") { const ids = [...new Set((body.ids || []).map(Number).filter((item) => Number.isInteger(item) && item > 0))].slice(0, 50); if (!ids.length) return Response.json({ error: "Selecione ao menos um produto." }, { status: 400 }); try { for (const productId of ids) { await approveProduct(productId); await db.insert(moderationAudit).values({ adminEmail: admin.email, entity: "product", entityId: productId, action: "approve" }); } } catch (error) { return Response.json({ error: error instanceof Error ? error.message : "Não foi possível aprovar os produtos." }, { status: 400 }); } return Response.json({ ok: true, approvedIds: ids }); }
    if (body.action === "approve") { try { await approveProduct(id); } catch (error) { return Response.json({ error: error instanceof Error ? error.message : "Não foi possível aprovar o produto." }, { status: 400 }); } }
