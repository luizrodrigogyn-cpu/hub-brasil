...
export async function POST(request:Request){
 ...
 const results=rows.map(item=>{const states:string[]=JSON.parse(item.serviceStates||"[]"),services:string[]=JSON.parse(item.services||"[]");const reasons:string[]=[];let score=0;if(body.category&&item.category===body.category){score+=45;reasons.push("atua na categoria escolhida")}if(body.state&&(item.servesNationwide||states.includes(body.state))){score+=25;reasons.push(item.servesNationwide?"atendimento nacional":"atende seu estado")}if(body.service&&services.includes(body.service)){score+=20;reasons.push(`oferece ${body.service.toLowerCase()}`)}if(body.mode&&(item.serviceMode===body.mode||item.serviceMode==="both")){score+=10;reasons.push("modalidade compatível")}if(item.verificationStatus==="verified"){score+=10;reasons.push("fornecedor verificado")}return{id:item.id,name:item.company||item.name,category:item.category,city:item.city,state:item.state,score,reasons}}).filter(item=>item.score>0).sort((a,b)=>b.score-a.score).slice(0,8);
