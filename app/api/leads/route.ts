...
export async function POST(request: Request) {
  try {
    ...
    const requestedRole = body.role === "supplier" ? "supplier" : "client";
    if (!user) {
      const returnTo = requestedRole === "supplier" ? "/?cadastro=fornecedor" : "/?cadastro=cliente";
      return Response.json({ error: "Faça login para continuar.", signIn: `/sign-in?return_to=${encodeURIComponent(returnTo)}` }, { status: 401 });
    }
    ...
    const submittedCategory = body.category?.trim() || null;
    ...
    const name = submittedName || (role === "supplier" ? company : null);
    const category = submittedCategory || (role === "supplier" ? selectedCategories[0] || null : null);
    const profileAddress = address || (role === "supplier" && city && state ? `${city}/${state}` : null);
    if (!name || !phone || (role === "client" && !profileAddress)) return Response.json({ error: "Preencha nome, telefone e endereço." }, { status: 400 });
    if (!/^\d{10,11}$/.test(phone.replace(/\D/g, ""))) return Response.json({ error: "Informe um telefone brasileiro válido com DDD." }, { status: 400 });
    if (role === "supplier" && (!company || !category || !selectedCategories.length || !city || !state)) return Response.json({ error: "Fornecedor deve informar empresa, WhatsApp, ao menos uma solução, cidade e estado." }, { status: 400 });
    ...
    if (role === "supplier" && cnpj && !isValidCnpj(cnpj)) return Response.json({ error: "O CNPJ informado não possui dígitos válidos." }, { status: 400 });
    ...
    const encrypted = { phoneEncrypted: await encryptPii(phone), instagramEncrypted: await encryptPii(instagram), emailEncrypted: await encryptPii(user.email), addressEncrypted: await encryptPii(profileAddress), cnpjEncrypted: await encryptPii(cnpj) };
