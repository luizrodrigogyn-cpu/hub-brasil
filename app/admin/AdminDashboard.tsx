...
export default function AdminDashboard() {
  ...
  return <>
    ...
    </div>{previewProduct && (() => { const specs = parseProductSpecs(previewProduct.specs); return <div className="product-preview-backdrop" role="presentation" onClick={() => setPreviewProduct(null)}><section className="product-preview-modal" role="dialog" aria-modal="true" aria-label={`Visualização de ${previewProduct.name}`} onClick={(event) => event.stopPropagation()}><button className="product-preview-close" onClick={() => setPreviewProduct(null)} aria-label="Fechar">×</button><div className="product-preview-media">{previewProduct.imageKey ? <img src={`/api/product-images?key=${encodeURIComponent(String(previewProduct.imageKey))}`} alt={`Foto de ${previewProduct.name}`} /> : <span>Produto sem foto</span>}</div><div className="product-preview-details"><div className="product-preview-heading"><div><small>PRÉVIA PARA A GESTÃO</small><h2>{previewProduct.name}</h2></div><span className={`status ${previewProduct.status}`}>{String(previewProduct.status)}</span></div><div className="product-preview-meta"><span><small>Fornecedor</small><strong>{previewProduct.supplierName}</strong></span><span><small>Categoria</small><strong>{previewProduct.category}</strong></span></div><section><h3>Descrição técnica</h3><p className="product-preview-description">{previewProduct.technicalDetails || "Não informada"}</p></section>{specs && <div className="product-preview-specs"><strong>Ficha técnica</strong><dl>{specs.technology?.length ? <div><dt>Tecnologia</dt><dd>{specs.technology.join(", ")}</dd></div> : null}{specs.application?.length ? <div><dt>Aplicação</dt><dd>{specs.application.join(", ")}</dd></div> : null}{specs.ip ? <div><dt>Proteção</dt><dd>{specs.ip}</dd></div> : null}{specs.battery ? <div><dt>Bateria</dt><dd>{specs.battery}</dd></div> : null}{specs.warranty ? <div><dt>Garantia</dt><dd>{specs.warranty}</dd></div> : null}</dl><div className="product-preview-badges">{specs.anatel && <span>✓ Homologado ANATEL</span>}{specs.features?.map((feature) => <span key={feature}>✓ {feature}</span>)}</div></div>}<div className="product-preview-attachments"><strong>Anexos e referências</strong>{previewProduct.manualUrl ? <div className="product-preview-document"><span>PDF</span><div><b>Manual técnico</b><small>Documento informado pelo fornecedor</small><div className="product-preview-document-actions"><a href={String(previewProduct.manualUrl)} target="_blank" rel="noopener noreferrer">Visualizar PDF ↗</a><a href={String(previewProduct.manualUrl)} download target="_blank" rel="noopener noreferrer">Baixar documento ↓</a></div></div></div> : <p>Nenhum manual anexado.</p>}{specs?.anatel && <div className="product-preview-certificate"><span>✓</span><div><b>Homologação ANATEL informada</b><small>Confirme o certificado antes da aprovação.</small></div></div>}</div></div></section></div>; })()}</>}
  </>;
}
