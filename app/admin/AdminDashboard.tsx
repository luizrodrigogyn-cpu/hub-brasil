...
export default function AdminDashboard() {
  ...
  return <>
    ...
    </div>{previewProduct && <div className="product-preview-backdrop" role="presentation" onClick={() => setPreviewProduct(null)}><section className="product-preview-modal" role="dialog" aria-modal="true" aria-label={`Visualização de ${previewProduct.name}`} onClick={(event) => event.stopPropagation()}><button className="product-preview-close" onClick={() => setPreviewProduct(null)} aria-label="Fechar">×</button><div className="product-preview-media">{previewProduct.imageKey ? <img src={`/api/product-images?key=${encodeURIComponent(String(previewProduct.imageKey))}`} alt={`Foto de ${previewProduct.name}`} /> : <span>Produto sem foto</span>}</div><div className="product-preview-details"><span className={`status ${previewProduct.status}`}>{String(previewProduct.status)}</span><small>PRÉVIA PARA A GESTÃO</small><h2>{previewProduct.name}</h2><p><strong>Fornecedor:</strong> {previewProduct.supplierName}</p><p><strong>Categoria:</strong> {previewProduct.category}</p><p><strong>Descrição técnica:</strong> {previewProduct.technicalDetails || "Não informada"}</p><p><strong>Valor médio:</strong> {previewProduct.averagePrice || "Não informado"}</p>{previewProduct.specs && <div className="product-preview-specs"><strong>Ficha técnica</strong><pre>{String(previewProduct.specs)}</pre></div>}{previewProduct.manualUrl && <a href={String(previewProduct.manualUrl)} target="_blank" rel="noreferrer">Abrir manual informado ↗</a>}</div></section></div>}</>}
  </>;
}
