  ...
  return (
    <div className="app-shell">
      ...
      <main>
        ...
        {view === "supplier-dashboard" && <section className="supplier-dashboard"><div className="page-heading"><div><span className="eyebrow">PAINEL DA EMPRESA</span><h1>{supplierCompany || "Minha empresa"}</h1><p>Publique soluções, acompanhe oportunidades e mantenha seu perfil atualizado.</p></div><div className="dashboard-actions"><button className="event-create" onClick={() => setEditingCompany(true)}>Editar empresa</button><button className="event-create" onClick={() => { setView("messages"); loadMessages(); }}>Mensagens</button></div></div>{previewMode === "supplier" ? <div className="approval-banner"><strong>Modo de visualização</strong><span>Você está vendo a experiência do fornecedor. Cadastros estão desativados nesta visualização.</span></div> : !supplierApproved && <div className="approval-banner"><strong>Cadastro em análise</strong><span>O gestor precisa validar seu telefone e aprovar sua empresa antes da primeira publicação.</span></div>}{Boolean(dashboard.supplierStats?.newLeads) && <div className="lead-alert"><strong>🔔 {dashboard.supplierStats?.newLeads} lead{(dashboard.supplierStats?.newLeads || 0) > 1 ? "s" : ""} novo{(dashboard.supplierStats?.newLeads || 0) > 1 ? "s" : ""} nas últimas 48h</strong><span>Responda rápido — fornecedores com resposta ágil aparecem melhor posicionados no diretório.</span></div>}<div className="metric-grid"><article><span>◉</span><strong>{dashboard.supplierMetrics?.profile_view || 0}</strong><small>Visualizações do perfil</small></article><article><span>◌</span><strong>{dashboard.supplierMetrics?.whatsapp_click || 0}</strong><small>Cliques no WhatsApp</small></article><article><span>↗</span><strong>{dashboard.supplierMetrics?.website_click || 0}</strong><small>Cliques no site</small></article><article><span>✦</span><strong>{dashboard.supplierMetrics?.quote_request || 0}</strong><small>Solicitações recebidas</small></article><article><span>☆</span><strong>{dashboard.supplierMetrics?.favorite || 0}</strong><small>Favoritos recebidos</small></article>{dashboard.supplierStats && dashboard.supplierStats.totalReceived > 0 && <article><span>%</span><strong>{dashboard.supplierStats.acceptanceRate}%</strong><small>Taxa de aceite ({dashboard.supplierStats.totalResponded}/{dashboard.supplierStats.totalReceived} cotações)</small></article>}</div><div className="management-grid"><button disabled={!supplierApproved || Boolean(previewMode)} onClick={() => setProductFormOpen(true)}><span>▣</span><strong>Cadastrar produto</strong><small>{previewMode ? "Disponível para fornecedores aprovados" : supplierApproved ? "Foto, categoria, especificações, aplicação e diferenciais" : "Disponível após aprovação"}</small></button><button disabled={!supplierApproved || Boolean(previewMode)} onClick={() => setEventFormOpen(true)}><span>★</span><strong>Cadastrar evento</strong><small>{previewMode ? "Disponível para fornecedores aprovados" : supplierApproved ? "Local, data e link de inscrição" : "Disponível após aprovação"}</small></button><button onClick={() => setView("products")}><span>⌕</span><strong>Ver produtos publicados</strong><small>Acompanhe seus cards e especificações no catálogo.</small></button></div>{(dashboard.supplierQuotes || []).length > 0 && <section className="dashboard-requests"><h2>Oportunidades recentes</h2>{dashboard.supplierQuotes?.map((quote) => <article key={quote.id} className={quote.isNewLead ? "is-new" : ""}><strong>{quote.protocol}{quote.isNewLead && <span className="new-tag">NOVO</span>}</strong><span>{quote.category} · {quote.application}</span>{quote.status === "responded" ? <small className="quote-outcome responded">✓ Respondida</small> : quote.status === "declined" ? <small className="quote-outcome declined">Recusada</small> : <div className="quote-actions-inline"><button disabled={Boolean(quoteActionBusy)} onClick={() => respondQuote(quote.id, "responded")}>{quoteActionBusy === `respond-${quote.id}` ? "…" : "Marcar como respondida"}</button><button disabled={Boolean(quoteActionBusy)} className="ghost" onClick={() => { if (window.confirm("Recusar esta cotação?")) respondQuote(quote.id, "declined"); }}>Recusar</button></div>}</article>)}</section>}</section>}

        {view === "client-dashboard" && <section className="client-dashboard"><div className="page-heading"><div><span className="eyebrow">MEUS PEDIDOS</span><h1>Minhas cotações</h1><p>Acompanhe o status de cada solicitação enviada e quantos fornecedores já responderam.</p></div><button className="event-create" onClick={openQuoteRequest}>＋ Nova cotação</button></div>{(dashboard.clientQuotes || []).length === 0 ? <div className="events-empty"><strong>Nenhuma cotação enviada ainda</strong><p>Quando você solicitar uma cotação a fornecedores aprovados, o status de cada pedido aparecerá aqui.</p><button className="event-create" onClick={() => setView("directory")}>Buscar fornecedores</button></div> : <div className="quote-status-grid">{dashboard.clientQuotes?.map((quote) => { const statusLabel = quote.recipientsResponded > 0 && quote.recipientsCompleted === quote.recipientsTotal ? "Todas as respostas recebidas" : quote.recipientsResponded > 0 ? `Propostas recebidas (${quote.recipientsResponded} de ${quote.recipientsTotal})` : quote.recipientsTotal > 0 && quote.recipientsCompleted === quote.recipientsTotal ? "Encerrada sem proposta" : "Aguardando propostas"; const statusClass = quote.recipientsResponded > 0 || (quote.recipientsTotal > 0 && quote.recipientsCompleted === quote.recipientsTotal) ? "responded" : "waiting"; const hours = Math.max(0, Math.round((Date.now() - new Date(quote.createdAt).getTime()) / 3600000)); const sentLabel = hours < 1 ? "enviado agora" : hours < 24 ? `enviado há ${hours}h` : `enviado há ${Math.round(hours / 24)} dia${Math.round(hours / 24) > 1 ? "s" : ""}`; return (<article className="quote-status-card" key={quote.id}><div className="quote-status-head"><strong>{quote.protocol}</strong><span className={`status-pill ${statusClass}`}>{statusLabel}</span></div><p>{quote.category} · {quote.application}</p><small>⌖ {quote.city}, {quote.state} · {sentLabel}</small>{quote.status === "closed" ? <small className="quote-outcome">Cotação encerrada</small> : <button className="ghost close-quote-btn" disabled={Boolean(quoteActionBusy)} onClick={() => { if (window.confirm("Encerrar esta cotação? Os fornecedores não poderão mais responder.")) closeQuoteRequest(quote.id); }}>{quoteActionBusy === `close-${quote.id}` ? "…" : "Encerrar cotação"}</button>}</article>); })}</div>}</section>}
