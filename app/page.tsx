...
type CreditEntry = { id:number; amount:number; direction:string; ruleKey:string; note?:string|null; createdAt:string; status?:string };
type SupplierDashboardData = {
  ...
  credits?: { wallet?: { availableBalance?: number; totalEarned?: number; totalUsed?: number }; ledger?: CreditEntry[] } | null;
};
...
export default function Home() {
  const [view, setView] = useState<"map" | "solutions" | "directory" | "supplier" | "supplier-entry" | "events" | "products" | "news" | "about" | "supplier-dashboard" | "supplier-credits" | "supplier-performance" | "client-dashboard" | "how-it-works" | "messages">("map");
  ...
  const [supplierRegistrationStep, setSupplierRegistrationStep] = useState<1 | 2>(1);
  ...
  const supplierCreditEntries = dashboard.credits?.ledger || [];
  const latestSupplierCredit = supplierCreditEntries.find((entry) => entry.direction === "credit" && entry.amount > 0);
  const formatResponseTime = (minutes?: number | null) => minutes == null ? "Sem dados" : minutes < 60 ? `${minutes} min` : `${(minutes / 60).toFixed(minutes >= 600 ? 0 : 1)} h`;
  ...
  const uncoveredCategory = (dashboard.profile?.categories || []).find((profileCategory) => !supplierProducts.some((product) => displayCategory(product.category) === displayCategory(profileCategory)));
  const supplierInsights = [
    uncoveredCategory ? {
      id: `insight-category-${uncoveredCategory}`,
      label: "Uma melhoria rápida",
      title: `Sua vitrine ainda não mostra ${displayCategory(uncoveredCategory)}`,
      reason: "Essa solução está no perfil da empresa, mas ainda não aparece no catálogo publicado.",
      action: "Adicionar produto",
      kind: "product",
    } : null,
    (dashboard.supplierStats?.quoteRequests7d || 0) > 0 && (dashboard.supplierStats?.responseRate7d || 0) >= 80 ? {
      id: "insight-response",
      label: "Bom ritmo comercial",
      title: "Sua resposta está rápida esta semana",
      reason: `${dashboard.supplierStats?.quoteResponses7d || 0} de ${dashboard.supplierStats?.quoteRequests7d || 0} oportunidades receberam resposta nos últimos 7 dias.`,
      action: "Ver performance",
      kind: "performance",
    } : null,
  ].filter(Boolean).slice(0, 3) as Array<{ id:string; label:string; title:string; reason:string; action:string; kind:string; opportunity?: typeof supplierOpportunities[number] }>;
    ...
    <div className="app-shell">
      ...
      <main>
        {view === "supplier-dashboard" && (
          <section className={`supplier-cockpit${dashboardLoading ? " is-loading" : ""}`} aria-busy={dashboardLoading}>
            ...
            <section className="opportunity-feed" id="supplier-opportunities"><div className="cockpit-section-heading"><div><span className="eyebrow">NEGÓCIOS COMPATÍVEIS</span><h2>Oportunidades</h2></div><p>Ordenadas por urgência e compatibilidade com sua empresa.</p></div>{supplierOpportunities.length ? <div className="opportunity-list">{supplierOpportunities.map((opportunity) => { const hours = Math.max(0, Math.round((Date.now() - new Date(opportunity.createdAt).getTime()) / 3600000)); const received = hours < 1 ? "Recebida agora" : hours < 24 ? `Recebida há ${hours}h` : `Recebida há ${Math.round(hours / 24)} dia${Math.round(hours / 24) > 1 ? "s" : ""}`; return <article key={opportunity.id} className={opportunity.status === "sent" ? "pending" : "answered"}><div className="opportunity-score"><strong>{opportunity.match.score}%</strong><small>compatível</small></div><div className="opportunity-copy"><div><span className="status-pill">{opportunity.status === "sent" ? "NOVA OPORTUNIDADE" : opportunity.status === "responded" ? "RESPONDIDA" : "RECUSADA"}</span><small>{received}</small></div><h3>{opportunity.category}</h3><p>{opportunity.quantity ? `${opportunity.quantity} unidades · ` : ""}{[opportunity.city, opportunity.state].filter(Boolean).join(" · ") || "Localização a confirmar"}</p><small>{opportunity.application || "Aplicação a confirmar"}</small></div><button className="opportunity-open" onClick={() => openSupplierOpportunity(opportunity)}>Ver detalhes <span>→</span></button></article>; })}</div> : <div className="cockpit-empty"><strong>Sua próxima oportunidade aparecerá aqui.</strong><p>Enquanto isso, deixe seu perfil pronto para ser encontrado.</p><button onClick={() => setEditingCompany(true)}>Melhorar meu perfil</button></div>}</section>
            <div className="cockpit-kpis" aria-label="Indicadores comerciais principais"><article><span>NOVAS OPORTUNIDADES</span><strong>{pendingSupplierOpportunities.length}</strong><small>{pendingSupplierOpportunities.length ? "aguardando sua atenção" : "nenhuma pendência agora"}</small></article><article><span>TAXA DE RESPOSTA</span><strong>{dashboard.supplierStats?.responseRate7d || 0}%</strong><small>nos últimos 7 dias</small></article><article><span>INTERAÇÕES NO PERFIL</span><strong>{supplierInteractions}</strong><small>contatos e cliques recentes</small></article><article className="credit-kpi"><span>HUB CRÉDITOS</span><strong>{Number(dashboard.credits?.wallet?.availableBalance || 0).toLocaleString("pt-BR")}</strong><small>{latestSupplierCredit ? `+${latestSupplierCredit.amount} · ${latestSupplierCredit.note || "Crédito recebido"}` : "saldo disponível"}</small><button onClick={() => setView("supplier-credits")}>Ver extrato →</button></article></div>
            <section className="for-you-section"><div className="cockpit-section-heading"><div><span className="eyebrow">PRÓXIMOS PASSOS</span><h2>Para você</h2></div><p>As ações de maior impacto agora.</p></div>{supplierPriorities.length ? <div className="for-you-grid">{supplierPriorities.map((priority) => <article key={priority.id} className={`priority-card ${priority.kind}`}><span>{priority.eyebrow}</span><h3>{priority.title}</h3><p>{priority.copy}</p><button onClick={() => { if (priority.kind === "product") openSupplierProductForm(); else if (priority.kind === "profile") setEditingCompany(true); else { const own = suppliers.find((item) => item.id === dashboard.profile?.id || item.name === supplierCompany); if (own) { setSelectedSupplier(own); setView("supplier"); } } }}>{priority.action} <i>→</i></button></article>)}</div> : <div className="cockpit-empty"><strong>Tudo em dia por aqui.</strong><p>Seu perfil está pronto para receber novas oportunidades.</p></div>}</section>
            <section className="profile-progress-card" aria-label="Progresso do perfil da empresa"><div className="profile-progress-copy"><span className="eyebrow">SEU PERFIL PÚBLICO</span><h2>Seu perfil está {supplierCompleteness}% completo</h2><p>Uma melhoria de cada vez, sem formulários longos.</p></div><strong>{supplierCompleteness}%</strong><div className="profile-progress-track"><i style={{ width: `${supplierCompleteness}%` }} /></div><div className="profile-progress-action"><div><small>PRÓXIMO PASSO</small><b>{supplierProfileNextAction.title}</b><span>{supplierProfileNextAction.copy}</span></div><button onClick={() => supplierProfileNextAction.kind === "product" ? openSupplierProductForm() : setEditingCompany(true)}>{supplierProfileNextAction.action} <i>→</i></button></div></section>
            {supplierInsights.length > 0 && <section className="commercial-intelligence" aria-label="Recomendações comerciais"><div className="cockpit-section-heading"><div><span className="eyebrow">MELHORE SEUS PRÓXIMOS MATCHES</span><h2>Ações recomendadas</h2></div><p>Recomendações explicáveis, baseadas apenas nos seus dados reais.</p></div><div className="intelligence-grid">{supplierInsights.map((insight) => <article key={insight.id}><span>{insight.label}</span><h3>{insight.title}</h3><p>{insight.reason}</p><button onClick={() => { if (insight.kind === "product") openSupplierProductForm(); else setView("supplier-performance"); }}>{insight.action} <i>→</i></button></article>)}</div></section>}
        ...
        {view === "supplier-credits" && (
          <section className="supplier-retention-page credit-center">
            <header className="retention-heading"><div><span className="eyebrow">HUB CRÉDITOS</span><h1>Visibilidade conquistada</h1><p>Seus créditos reconhecem ações que fortalecem sua presença no Hub.</p></div><button className="event-create" onClick={() => setView("supplier-dashboard")}>← Voltar ao painel</button></header>
            <div className="credit-balance-card"><div><small>SALDO DISPONÍVEL</small><strong>{Number(dashboard.credits?.wallet?.availableBalance || 0).toLocaleString("pt-BR")}</strong><span>Hub Créditos</span></div>{latestSupplierCredit && <p><b>+{latestSupplierCredit.amount}</b><span>{latestSupplierCredit.note || "Crédito recebido"}</span></p>}</div>
            <div className="credit-summary"><article><span>Total conquistado</span><strong>{Number(dashboard.credits?.wallet?.totalEarned || 0).toLocaleString("pt-BR")}</strong></article><article><span>Créditos utilizados</span><strong>{Number(dashboard.credits?.wallet?.totalUsed || 0).toLocaleString("pt-BR")}</strong></article></div>
            <section className="credit-history"><div className="retention-section-title"><div><span className="eyebrow">MOVIMENTAÇÕES</span><h2>Histórico recente</h2></div><p>Somente ações reais registradas na plataforma.</p></div>{supplierCreditEntries.length ? <div>{supplierCreditEntries.slice(0, 10).map((entry) => <article key={entry.id}><span className={entry.direction === "credit" ? "credit-positive" : "credit-negative"}>{entry.direction === "credit" ? "+" : "−"}{Math.abs(entry.amount).toLocaleString("pt-BR")}</span><div><strong>{entry.note || "Movimentação de Hub Créditos"}</strong><small>{new Date(entry.createdAt).toLocaleDateString("pt-BR", { day:"2-digit", month:"short", year:"numeric" })}</small></div></article>)}</div> : <div className="retention-empty"><strong>Suas conquistas aparecerão aqui.</strong><p>Complete o perfil, publique produtos aprovados e responda oportunidades para movimentar seu saldo.</p></div>}</section>
            <p className="credit-disclaimer">Hub Créditos são pontos promocionais internos, sem valor monetário, saque ou conversão em dinheiro. Eles não alteram avaliações ou o Hub Score.</p>
          </section>
        )}
        {view === "supplier-performance" && (
          <section className="supplier-retention-page performance-center">
            <header className="retention-heading"><div><span className="eyebrow">PERFORMANCE</span><h1>Seu ritmo comercial</h1><p>Indicadores reais para entender oportunidades, respostas e interesse na sua vitrine.</p></div><button className="event-create" onClick={() => setView("supplier-dashboard")}>← Voltar ao painel</button></header>
            <section className="performance-period"><div className="retention-section-title"><div><span className="eyebrow">ÚLTIMOS 7 DIAS</span><h2>Resposta às oportunidades</h2></div><p>Uma leitura objetiva do período recente.</p></div><div className="performance-grid"><article><span>Oportunidades recebidas</span><strong>{dashboard.supplierStats?.quoteRequests7d || 0}</strong></article><article><span>Respostas enviadas</span><strong>{dashboard.supplierStats?.quoteResponses7d || 0}</strong></article><article><span>Taxa de resposta</span><strong>{dashboard.supplierStats?.responseRate7d || 0}%</strong></article><article><span>Tempo médio de resposta</span><strong>{formatResponseTime(dashboard.supplierStats?.avgResponseMinutes7d)}</strong></article></div></section>
            <section className="performance-period"><div className="retention-section-title"><div><span className="eyebrow">VISÃO AMPLIADA</span><h2>Presença e interesse</h2></div><p>Dados de apoio, fora do cockpit principal.</p></div><div className="performance-grid secondary"><article><span>Oportunidades em 30 dias</span><strong>{dashboard.supplierStats?.quoteRequests30d || 0}</strong></article><article><span>Visualizações do perfil</span><strong>{dashboard.supplierMetrics?.profile_view || 0}</strong><small>janela recente</small></article><article><span>Produtos visualizados</span><strong>{dashboard.supplierMetrics?.product_view || 0}</strong><small>janela recente</small></article><article><span>Interações comerciais</span><strong>{supplierInteractions}</strong><small>WhatsApp, site e contatos</small></article></div></section>
            <section className="performance-insight"><span>LEITURA DO PERÍODO</span><strong>{(dashboard.supplierStats?.quoteRequests7d || 0) === 0 ? "Ainda não há oportunidades recentes para comparar." : (dashboard.supplierStats?.responseRate7d || 0) >= 80 ? "Sua taxa de resposta está consistente nesta semana." : "Responder as oportunidades pendentes pode melhorar sua taxa de resposta."}</strong><p>Os indicadores são calculados somente a partir de atividades registradas no Hub Brasil.</p></section>
          </section>
        )}
        {view === "supplier-entry" && (
        ...
        {view === "supplier" && selectedSupplier && (
          <section className="profile-page">
            ...
            <div className="profile-columns">
              <div className="supplier-catalog"><span className="eyebrow">SOLUÇÕES DA EMPRESA</span><h2>Catálogo de produtos</h2><p className="supplier-catalog-intro">Conheça os equipamentos e soluções publicados por este fornecedor.</p>{products.filter((item) => (item.supplierId != null ? item.supplierId === selectedSupplier.id : item.supplierName === selectedSupplier.name)).length === 0 ? <div className="events-empty"><strong>Nenhum produto publicado</strong><p>Os produtos aprovados deste fornecedor aparecerão aqui.</p></div> : <div className="product-grid">{products.filter((item) => (item.supplierId != null ? item.supplierId === selectedSupplier.id : item.supplierName === selectedSupplier.name)).map((item) => <article className="product-card" key={item.id}><div className="product-visual">{item.imageUrl ? <img src={item.imageUrl} alt={item.name} /> : <div className="device"></div>}</div><div className="product-card-copy"><span className="category">{displayCategory(item.category)}</span><h3>{item.name}</h3><p>Ficha técnica, aplicações e diferenciais</p><button onClick={() => openProduct(item)}>Explorar produto <span>→</span></button></div></article>)}</div>}</div>
      ...
      {selectedProduct && (
        <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelectedProduct(null); }}>
          <section className="access-modal product-detail" role="dialog" aria-modal="true">
            ...
            <div className="detail-photo">{selectedProduct.imageUrl ? <img src={selectedProduct.imageUrl} alt={selectedProduct.name} /> : <span>Produto sem foto</span>}</div><div className="product-detail-scroll">
      ...
      {registerOpen && (
        <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setRegisterOpen(false); }}>
          <section className={`access-modal${registrationRole === "supplier" ? " supplier-registration-modal" : ""}`} role="dialog" aria-modal="true" aria-labelledby="access-title">
            ...
            <span className="eyebrow">{registrationRole === "supplier" ? "CADASTRO DE FORNECEDOR" : "ACESSO PROFISSIONAL"}</span>
            <h2 id="access-title">{registrationRole === "supplier" ? "Comece com o essencial" : "Identifique-se para acessar"}</h2>
            <p>{registrationRole === "supplier" ? "Seu perfil será analisado antes de aparecer publicamente. Você poderá completar a vitrine depois." : "Precisamos conhecer quem está buscando fornecedores. É rápido e seus dados ficam protegidos."}</p>
            <div className="role-selector"><button className={registrationRole === "client" ? "active" : ""} onClick={() => { setRegistrationRole("client"); setSupplierRegistrationStep(1); }}><strong>Sou usuário</strong><span>Quero encontrar e avaliar fornecedores</span></button><button className={registrationRole === "supplier" ? "active" : ""} onClick={() => { setRegistrationRole("supplier"); setSupplierRegistrationStep(1); }}><strong>Sou fornecedor</strong><span>Quero receber oportunidades</span></button></div>
            <form onSubmit={register}>
              ...
              {registrationRole === "client" ? <>
                ...
                <div className="registration-progress supplier-steps"><span className={supplierRegistrationStep === 1 ? "active" : "complete"}><i>1</i> Empresa</span><b></b><span className={supplierRegistrationStep === 2 ? "active" : ""}><i>2</i> Atuação</span></div>
                <div className={`supplier-registration-step${supplierRegistrationStep === 1 ? " active" : ""}`} aria-hidden={supplierRegistrationStep !== 1}>
                  <span className="eyebrow">INFORMAÇÕES ESSENCIAIS</span>
                  <h3>Conte sobre sua empresa</h3>
                  <p>Usaremos estes dados para validar o cadastro e preparar seu perfil.</p>
                  <label>CNPJ <small>Opcional · preenche os dados cadastrais automaticamente</small><input name="cnpj" inputMode="numeric" autoComplete="off" placeholder="00.000.000/0000-00" maxLength={18} onChange={lookupSupplierCnpj} /></label>
                  ...
                  <label>Nome da empresa<input name="company" required autoComplete="organization" placeholder="Ex.: Nivion Tecnologia" /></label>
                  <WhatsAppField />
                ...
                <div className={`supplier-registration-step${supplierRegistrationStep === 2 ? " active" : ""}`} aria-hidden={supplierRegistrationStep !== 2}>
                  <span className="eyebrow">SUA ATUAÇÃO</span>
                  <h3>O que sua empresa oferece?</h3>
                  <p>Escolha as soluções principais agora. Você poderá ampliar sua vitrine depois.</p>
                  <fieldset className="solution-selector solution-cards"><legend>Categoria principal <small>Escolha uma ou mais.</small></legend>{solutionCategories.map((item) => <label className="check" key={item.name}><input name="categories" type="checkbox" value={item.name} /><span aria-hidden="true">{item.icon}</span>{item.title}</label>)}</fieldset>
                  <details className="optional-profile-details"><summary>Adicionar logo agora <small>Opcional</small></summary><label className="optional-upload">Logo da empresa <small>PNG, JPG ou WebP</small><input name="logo" type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => setSupplierLogoSelected(Boolean(event.currentTarget.files?.length))} /></label>{supplierLogoSelected && <label className="consent logo-consent"><input name="logoConsent" type="checkbox" required /> <span>Declaro que possuo autorização para utilizar e divulgar esta marca/logotipo no Hub Brasil.</span></label>}</details>
                  <p className="progressive-note"><strong>O restante fica para depois.</strong> Descrição, site, regiões atendidas e produtos serão adicionados um passo por vez no cockpit.</p>
              ...
              {registrationRole === "client" && <label className="consent"><input type="checkbox" required /> <span>Li e concordo com a <a href="/privacidade" target="_blank">Política de Privacidade</a> e os <a href="/termos" target="_blank">Termos de Uso</a>.</span></label>}
              {registrationRole === "supplier" && supplierRegistrationStep === 2 && <button className="supplier-step-back" type="button" onClick={() => setSupplierRegistrationStep(1)}>← Voltar</button>}
              {registrationRole === "supplier" && supplierRegistrationStep === 1 ? <button className="primary full" type="button" onClick={(event) => { const form = event.currentTarget.form; const requiredNames = ["company", "phone", "city", "state"]; const invalid = requiredNames.map((name) => form?.elements.namedItem(name)).find((field) => field instanceof HTMLInputElement || field instanceof HTMLSelectElement ? !field.checkValidity() : false); if (invalid instanceof HTMLInputElement || invalid instanceof HTMLSelectElement) { invalid.reportValidity(); return; } setSupplierRegistrationStep(2); }}>Continuar <span>→</span></button> : <button className="primary full" type="submit">{registrationRole === "supplier" ? "Enviar para análise" : "Liberar meu acesso"} <span>→</span></button>}
            </form>
